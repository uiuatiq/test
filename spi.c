...
int main(void){
	...
	/* Sample data */
/*char *data = "ox2c8018,0x8008029,0x4e42,0x4b3,0xec803c,0x580005";*/
          int data0 = 0x002c8018;
          int data1 = 0x08008029;
          int data2 = 0x00004e42;
          int data3 = 0x000004b3;
          int data4 = 0x00ec803c;
          int data5 = 0x00580005;
	...
	/* Write some sample data */
       ...
       //################
      /*write R0 data*/
        if(write_spi(data0,4) < 0){
                printf("Write data0 to spi failed. Error: %s\n",strerror(errno));
                return -1;
        }

      /*writeR1*/
        if(write_spi(data1,4) < 0){
               printf("Write data1 to spi failed. Error: %s\n",strerror(errno));
               return -1;
        }

     /* Write R2*/
        if(write_spi(data2, 4) < 0){
               printf("Write data2 to spi failed. Error: %s\n",strerror(errno));
               return -1;
        }
      /*Write R3*/
        if(write_spi(data3, 4) < 0){
               printf("write data3 to spi failed. Error : %s\n",strerror(errno));
               return -1;
        }
   
       /*Write R4*/
         if(write_spi(data4, 4) < 0){
               printf("write data4 to spi failed. Error: %s\n",strerror(errno));
               return -1;
         }
      /* Write R5*/
         if(write_spi(data5, 4) < 0){
                printf("write data5 to soi failed. Error: %s\n",strerror(errno));
                return -1;
         }
